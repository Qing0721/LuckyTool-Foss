...
object HookAppDetails : YukiBaseHooker() {
    ...
    override fun onHook() {
        ...
        val clazz = "com.oplus.settings.feature.appmanager.AppInfoFeature".toClassOrNull(appClassLoader) ?: return
        clazz.method { name = "setAppLabelAndIcon"; paramCount = 1 }.ignored().hook {
            after {
                val mRootView = clazz.field { name = "mRootView" }.ignored().get(instance).any() as? View
                    ?: return@after
                val appButtonsPreferenceController = args.getOrNull(0) ?: return@after
                val instrumentedPreferenceFragment = runCatching {
                    appButtonsPreferenceController.javaClass.getDeclaredField("mFragment")
                        .apply { isAccessible = true }
                        .get(appButtonsPreferenceController)
                }.getOrNull() ?: return@after
                val packageInfo = runCatching {
                    var c: Class<*>? = instrumentedPreferenceFragment.javaClass
                    while (c != null) {
                        val f = c.declaredFields.firstOrNull { PackageInfo::class.java.isAssignableFrom(it.type) }
                        if (f != null) {
                            f.isAccessible = true
                            return@runCatching f.get(instrumentedPreferenceFragment) as? PackageInfo
                        }
                        c = c.superclass
                    }
                    null
